...
class Clusterize {
  ...
  checkV(v) {
    ...
    for (let i=0, max=v.length; i<max; ++i) {
      if (!_.isArray(v[i])) {
        ...
        for (let j=0, max=v[i].length; j<max; ++j) {
          v[i][j] = Number(v[i][j]);
  ...
  clustering() {
    for (let j=0, max=this.groups.length; j<max; ++j) this.groups[j].initCluster();
    for (let i=0, max=this.v.length; i<max; ++i) {
      let min = this.groups[0].distances[i];
      let indexGroup = 0;
      for (let j=1, max=this.groups.length; j<max; ++j) {
...
class Group {
  ...
  defineCentroid(self) {
    ...
    if (this.centroid && this.cluster.length>0) {
      ...
      let i = Math.floor(Math.random() * self.indexes.length);
      ...
      if (!_.isArray(self.v[this.centroidIndex])) { // only one dimension
        ...
        for (let i=0, max=self.v[this.centroidIndex].length; i<max; ++i) {
    ...
    if (this.centroid.length == 0) {
      console.log('1. was passiert hier??');
    }
    return this;
  }
